...
#  println(1 ./etp[26])
  ...
  errY = evolving_error(errY,ph,eph,pe,epe, posteriorM, IdDD ) #y must be D with no sqrt (!)
  errY = errY .* bigYD
...
#          err_extra = maximum(errY)
    ...
    p12 = Plots.scatter(2:1:26,reverse(1 ./etp[1:25] ), yerr = errY, markerstrokecolor=1, size=(300,300), legends=:none,color=1, shape=:+, markersize = 5)
      ...
      Plots.yaxis!(minorticks=5)
      asymp = string.(round.(1/etp[26]; digits=2))
      annotate!(-1, 1/etp[26], Plots.text(asymp, :red, :right, 14))
if ITALIC == 1
      annotate!(26, 73.8, Plots.text(notaB,:right, 20,  "Times Italic"))
else
      annotate!(26, 73.8, Plots.text(notaB,:right, 20, "Helvetica Bold")) 
end
...
#    println(errY)
  ...
  annotate!(-1, 1 ./etp[26], Plots.text(asymp, :red, :right, 14))
if ITALIC == 1
  annotate!(25, 68.5, Plots.text(notaD,:right, 20, "Times Italic"))
else
  annotate!(25, 68.5, Plots.text(notaD,:right, 20, "Helvetica Bold")) 
end
...
if PLOTFIGS == 1
#    savefig("./SRfigHLCEM.png")
    savefig("./figHLCDMsoloY1-rev1-v1.png")
end

    print("GOING OUT ...")
#    return posteriorMcg,  posteriorMmotif
